...
Phase 8.1 已完成 LocalStorage v2 的安全恢复：非法盲注、损坏 AI profile/习惯记录和导入文件会逐字段修复并提示，合法配置与纯自定义 AI 池保持兼容。下一项 P2 是补充 Playwright WebKit 自动化矩阵，重点覆盖复盘界面、本地持久化、导出、响应式布局和键盘/输入边界。Review Lab v1.1 的决策快照仍需独立设计 v3 迁移，不能与浏览器矩阵混合。
...
- 现有 112 项单元测试和 13 项 Chromium E2E 继续通过
