...
import { compareEvaluations, evaluateBestHand } from "../../engine/evaluator/evaluator";
...
export function simulationDeck(request: EquityRequest): Card[] {
  const known = [...request.holeCards, ...request.board, ...(request.deadCards ?? [])];
...
export function estimateEquity(request: EquityRequest): EquityResult {
  if (request.opponents < 1 || request.opponents > 5) throw new Error("Opponent count must be 1 to 5");
  if (request.board.length > 5) throw new Error("Board cannot exceed five cards");
  ...
  if (cardsNeeded > baseDeck.length) throw new Error("Not enough unknown cards for simulation");
  let wins = 0;
  let ties = 0;
  let losses = 0;
  ...
  for (let iteration = 0; iteration < iterations; iteration += 1) {
    ...
    let cursor = 0;
    const opponentHands = Array.from({ length: request.opponents }, () => [deck[cursor++], deck[cursor++]]);
    const board = [...request.board, ...deck.slice(cursor, cursor + (5 - request.board.length))];
    const hero = evaluateBestHand([...request.holeCards, ...board]);
    const opponents = opponentHands.map((holeCards) => evaluateBestHand([...holeCards, ...board]));
    const comparisons = opponents.map((hand) => compareEvaluations(hero, hand));
    if (comparisons.every((value) => value > 0)) wins += 1;
    else if (comparisons.every((value) => value >= 0)) ties += 1;
    else losses += 1;
  }
  return { equity: (wins + ties * 0.5) / iterations, wins, ties, losses, iterations };
}

export async function estimateEquityAsync(request: EquityRequest): Promise<EquityResult> {
