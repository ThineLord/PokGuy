...
export function adaptPersonality(
  ...
  if (!user || user.sampleSize < 20 || base.adaptability <= 0) return { ...base };
  ...
  const boundedDelta = (adjustment: number) => clamp(adjustment, -0.12 * power, 0.12 * power);
  ...
  return {
    ...base,
    bluffFrequency: clamp(base.bluffFrequency + boundedDelta(excessiveFolds - excessiveCalls)),
    pfr: clamp(base.pfr + boundedDelta(excessiveFolds * 0.45 + loosePreflop * 0.35)),
    aggression: clamp(base.aggression + boundedDelta(passiveChecks - excessiveCalls * 0.35)),
    checkRaiseFrequency: clamp(base.checkRaiseFrequency + boundedDelta(frequentCbet)),
  };
}
...
export function smoothRate(
  ...
  if (observedOpportunities < 0 || observedSuccesses < 0) throw new Error("Counts cannot be negative");
  return (observedSuccesses + priorRate * priorWeight) / (observedOpportunities + priorWeight);
}
