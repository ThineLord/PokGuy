...
function nextActivePlayerId(state: BettingRoundState, fromSeat: number): string | null {
  const eligible = state.players
    .filter((player) => player.status === "active" && player.stack > 0)
    .sort((a, b) => a.seat - b.seat);
  if (eligible.length === 0) return null;
  return eligible.find((player) => player.seat > fromSeat)?.id ?? eligible[0].id;
}

export function isRoundComplete(state: BettingRoundState): boolean {
  const contenders = state.players.filter((player) => player.status !== "folded" && player.status !== "busted" && player.status !== "sitting-out");
...
export function applyBettingAction(
  ...
  if (action.type === "call") target += Math.min(validation.toCall, actor.stack);
  ...
  const fullRaise = target > previousBet && raiseIncrement >= state.minRaiseIncrement;

  let players = state.players.map((player): BettingPlayer => {
    if (player.id === playerId) {
      const remaining = player.stack - paid;
      return {
        ...player,
        stack: remaining,
        streetContribution: target,
        totalContribution: player.totalContribution + paid,
        status: action.type === "fold" ? "folded" : remaining === 0 ? "all-in" : player.status,
        acted: true,
        lastActedBet: Math.max(previousBet, target),
      };
    }
    if (fullRaise && player.status === "active") return { ...player, acted: false };
    return player;
  });

  const currentBet = Math.max(previousBet, target);
  const minRaiseIncrement = fullRaise ? raiseIncrement : state.minRaiseIncrement;
