...
- 默认决策使用 120 次轻量模拟；训练 UI 使用 240 次，并以牌面和剩余对手数生成稳定 seed，避免同一决策点因其他玩家行动而无意义跳动。
- 多人平局按实际并列人数分配 equity，例如四人共享公共牌最佳牌时每人计 25%，而不是固定按 50%。
...
每种人格会保存实际运行的 VPIP、PFR、3-bet、fold to 3-bet、c-bet、fold to c-bet、aggression factor、摊牌率、诈唬估计和河牌跟注率。设置页同时显示人格预设和已积累的实际样本。实际统计使用 20 手先验权重平滑，不会被第一两手直接推到 0% 或 100%。
